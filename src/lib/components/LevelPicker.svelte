  ...
  <div aria-labelledby="{id}-label" class="grid grid-cols-1 gap-2 sm:grid-cols-2">
    {#each levels as level (level.code)}
      ...
      <button
        ...
        class="flex flex-col items-start gap-0.5 rounded-xl border px-3 py-2.5 text-left transition-all duration-150
          {selected
      ...
      >
        <span class="text-sm font-semibold {selected ? 'text-indigo-700' : 'text-gray-900'} leading-tight">
          {level.name}
        </span>
        <span class="text-xs {selected ? 'text-indigo-600/80' : 'text-gray-500'} leading-snug">
          {level.description}
        </span>
      </button>
    {/each}
